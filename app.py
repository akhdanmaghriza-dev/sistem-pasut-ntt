...
# --- DATABASE FASE BULAN 2026 (Warna Ikon Disesuaikan dengan Legenda Kalender BMKG) ---
FASE_BULAN_2026 = {
    # JANUARI
...
with col_title:
    st.markdown("<h1 style='color: #1e3a8a; margin:0; padding:0; line-height:1.1;'>Prakiraan Pasang Surut Air Laut NTT</h1>", unsafe_allow_html=True)
    st.markdown("<h3 style='color: #475569; margin:0; padding:0; font-weight:normal;'>Stasiun Meteorologi Kelas III Maritim Tenau - Kupang</h3>", unsafe_allow_html=True)
...
with tab1:
    ...
    if tgl_selesai >= tgl_mulai and len(pilih_wilayah) > 0:
        ...
        if not df_tren.empty:
            # METRIK HIGHLIGHT EKSTREM GLOBAL
            ...
            with m1:
                ...
                <div style='background-color: #fef2f2; border-left: 5px solid #ef4444; padding: 10px; border-radius: 5px;'>
                    <p style='color: #991b1b; margin:0; font-weight: bold; font-size:13px;'>🔴 REKOR PASANG TERTINGGI (Acuan Datum: LAT)</p>
                    <h3 style='color: #b91c1c; margin:2px 0;'>{df_tren.loc[idx_max, 'Ketinggian']:.2f} m <span style='font-size:14px; font-weight:normal;'>({df_tren.loc[idx_max, 'Wilayah']})</span></h3>
                    <p style='color: #7f1d1d; margin:0; font-size:12px;'>Waktu: {df_tren.loc[idx_max, 'Waktu'].strftime('%d %b %Y | %H:00 WITA')}</p>
                </div>
            ...
            with m2:
                ...
                <div style='background-color: #eff6ff; border-left: 5px solid #2563eb; padding: 10px; border-radius: 5px;'>
                    <p style='color: #1e40af; margin:0; font-weight: bold; font-size:13px;'>🔵 REKOR SURUT TERENDAH (Acuan Datum: LAT)</p>
                    <h3 style='color: #1d4ed8; margin:2px 0;'>{df_tren.loc[idx_min, 'Ketinggian']:.2f} m <span style='font-size:14px; font-weight:normal;'>({df_tren.loc[idx_min, 'Wilayah']})</span></h3>
                    <p style='color: #1e3a8a; margin:0; font-size:12px;'>Waktu: {df_tren.loc[idx_min, 'Waktu'].strftime('%d %b %Y | %H:00 WITA')}</p>
                </div>
                """, unsafe_allow_html=True)
            ...
            for i, wil in enumerate(pilih_wilayah):
                ...
                # Detektor Puncak Harian Ganda (Dua Pasang Dua Surut Sehari)
                ...
                fig.add_trace(go.Scatter(
                    ...
                    textfont=dict(color="#dc2626", size=11), marker=dict(color='#ef4444', size=9, line=dict(color='white', width=1.5)), 
                ...
                fig.add_trace(go.Scatter(
                    ...
                    textfont=dict(color="#1d4ed8", size=11), marker=dict(color='#2563eb', size=9, line=dict(color='white', width=1.5)), 
            ...
            fig.update_layout(
                title=dict(text="<b>Grafik Tren Fluktuasi Ketinggian Air Laut Pasut NTT (WITA)</b>", font=dict(size=15, color='#1e293b')),
                ...
                template="plotly_white", height=600, margin=dict(t=50, b=30, l=40, r=40), hovermode="x unified"
            )
            ...
            for wil in pilih_wilayah:
                ...
                c1, c2 = st.columns(2)
                with c1:
                    st.markdown("<h5 style='color: #b91c1c; margin-bottom: 5px;'>⬆️ Jadwal Pasang Tertinggi (High Tide)</h5>", unsafe_allow_html=True)
                    st.dataframe(h.reset_index(drop=True), use_container_width=True)
                with c2:
                    st.markdown("<h5 style='color: #1d4ed8; margin-bottom: 5px;'>⬇️ Jadwal Surut Terendah (Low Tide)</h5>", unsafe_allow_html=True)
                    st.dataframe(l.reset_index(drop=True), use_container_width=True)

        else:
            st.warning("⚠️ Berkas data tidak ditemukan. Pastikan file Excel bulanan diletakkan di folder aplikasi dengan format nama yang benar (Contoh: 'Pasut_Juli.xlsx').")
    ...
    if len(tgl_cmp) > 0:
        ...
        if data_ditemukan:
            fig_cmp.update_layout(
                title=dict(text=f"<b>Analisis Komparasi Siklus Harian Pelabuhan {wil_cmp}</b>", font=dict(size=15, color='#1e293b')),
...
st.write("---")
st.markdown("""
<div style='text-align: center; color: grey; font-size: 11px; padding: 12px; background-color: #f8fafc; border-radius: 5px; border: 1px solid #e2e8f0;'>
